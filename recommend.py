...
def find_centroid(cluster):
    ...
    for r in cluster:
        latitude_sum += restaurant_location(r)[0]
        longitude_sum += restaurant_location(r)[1]
    latitude = latitude_sum/(len(cluster))
    longitude = longitude_sum/(len(cluster))
...
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xy = sum([(j-mean(xs)) * (k-mean(ys)) for j, k in zip(xs, ys)])
